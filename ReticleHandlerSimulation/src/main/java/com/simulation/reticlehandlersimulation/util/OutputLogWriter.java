...
public class OutputLogWriter {
    ...
    public void writeToLog(String reticleName, String message){
        String content = getMessageContent(reticleName, message);
        try {
            logger.info(content);
            //bufferedWriter.write(content);
        } catch (Exception ex) {
    ...
    private String getMessageContent(String reticleName, String message){
        if(message.equals(RETICLE_REWIND)){
            return "User rewound reticle "+ reticleName;
        }
        else{
            return "Reticle "+ reticleName+ " "+ message;
        }
    }
